...
echo "Your kit looks good.";
	echo "Verify the local hostname for this IDC. Leave blank for \"$IDC_HOSTNAME\"."
	read READ_HOSTNAME;
...
if [ $INSTALL_DB == 1 ]; then
	...
	while [ $MYSQL_PRIV_ACCESS == 0 ]; do
		echo "Input the MySQL server hostname; leave blank for localhost"
		read MYSQL_SERVER;
		...
		echo "Input a privileged MySQL username on that host. Leave blank for root."
		...
		echo "Input the password for the privileged account"
		stty -echo;
	...
	while [ $MYSQL_GOT_IDC_CREDENTIALS == 0 ]; do
		...
		echo "Input a MySQL username the IDC will use to connect. Leave blank for \"oscars\""
		...
		echo "Input the password for the IDC account"
		stty -echo;
		...
		if [ $? != 0 ]; then
			...
			echo "Input the password for the IDC account one more time:"
			stty -echo;
	...
	echo "Input the BSS database name. Leave blank for \"bss\"."
	read MYSQL_BSS_DBNAME;
	if [ ! $MYSQL_BSS_DBNAME ]; then			echo "  Using oscars";

		MYSQL_BSS_DBNAME="bss";
	fi
	echo "  Using $MYSQL_BSS_DBNAME . ";

	echo "Input the AAA database name. Leave blank for \"aaa\"."
...
#TODO: customize oscars.properties with MySQL data, generate ssh keys, etc
