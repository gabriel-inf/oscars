...
public abstract class BaseSDNTopologyService implements ISDNTopologyService {
	...
	public static List<SDNHop> extractSDNHops(List<CtrlPlaneHopContent> ogfHops) {
		...
		/* this is l2 specific, may went to move this to a subclass */
		String srcVlan = null;
		try {
			int count = 0;
			for (CtrlPlaneHopContent hop : ogfHops) {
				String dst = hop.getLink().getId();
				String dstVlan = extractVlan(hop);
				count++;
				
				System.out.println("src="+src +",dst="+dst);
				//special case for virtualized nodes with ports conecting them
				if("*".equals(NMWGParserUtil.getURNPart(dst, NMWGParserUtil.PORT_TYPE))){
				...
				if (NMWGParserUtil.compareURNPart(src.replaceAll("#\\d+", ""), dst.replaceAll("#\\d+", ""),
						NMWGParserUtil.NODE_TYPE) == 0) {
					SDNHop h = new SDNHop(src.replaceAll("#\\d+", ""), dst.replaceAll("#\\d+", ""));
					if(srcVlan != null && dstVlan != null){
					    h.setSrcVlan(srcVlan);
					    h.setDstVlan(dstVlan);
					}
	...
	public static String extractVlan(CtrlPlaneHopContent hop){
	    String vlan = null;
	    if(hop.getLink().getSwitchingCapabilityDescriptors() != null && 
	        ...
	        vlan = hop.getLink().getSwitchingCapabilityDescriptors().getSwitchingCapabilitySpecificInfo().getVlanRangeAvailability();
            }
