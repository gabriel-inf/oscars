...
try {
    if (args.length == 1) {
        if (args[0].equals("-h") || args[0].equals("-help")) {
            System.out.println(" Usage: java KeyStoreImport KEYSTORE CERTS KEY ALIAS \n" +
                    "\t KEYSTORE is the name of the file containing the Java keystore \n"  +
                    "\t CERTS is the name of a file containing a chain of concatenated \n" +
                    "\t      DER-encoded X.509 certificates \n" +
                    "\t KEY is the name of a file containing a DER-encoded PKCS#8 RSA \n" +
                    "\t       private key \n" +
            "\t ALIAS is the alias for the private key entry in the keystore \n" + 
            "\t use openssl to convert certificate and key files into the expected format ");
            System.exit(0);
        }
    }
    if (args.length < 4) {
        System.out
        .println("usage:  [-h] keyStoreFileName CertificateChainFileName, PrivateKeyFileName, entryAlias");
        System.exit(1);
    }
