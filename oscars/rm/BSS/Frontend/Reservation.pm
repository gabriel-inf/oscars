...
# reservation_check_disabled:  DB handling for network resource reservation 
##### sub reservation_check_disabled
...
sub reservation_check_disabled
{
  ...
  foreach $key ( 'origin', 'destination' ) {
      my @temp_node_lookup_result;
      if ( $key eq 'origin' ) {
            # connect to the Indianapolis node (which is the closest to the
            # login server) and run traceroute to the origin to find out the
            # closest node to the origin
          @temp_node_lookup_result = &node_lookup( 'ipls', $args_ref->{$key} );
      }
      else {
          # connect to the node that is closest to the origin
          # and run traceroute from that node to the destination host
          @temp_node_lookup_result = &node_lookup( $closest_node{'origin'}, $args_ref->{$key} );
      }

      if ( $temp_node_lookup_result[0] != 1 ) {
          if ( $temp_node_lookup_result[0] eq 'command_fail' ) {
              return( 1, '[ERROR] An error has occurred while traversing the network path. Please try again later.' );
          }
          elsif ( $temp_node_lookup_result[0] eq 'non_abilene' ) {
              return( 1, '[ERROR] This origin-destination path does not go through the network. Please check the origin and destination IP addresses and try again.' );
          }
      }
      else {
          $closest_node{$key} = $temp_node_lookup_result[1];
      }
  }
=cut
	...
	### [notes on 6/14/2004]
	# [note] look at 1) and 3) only, since we now treat the entire network as a single path
	...
	if ( $conflict_status )
	{
		...
		return( 1, '[ERROR] The available bandwidth limit has been reached between ' . $Conflicted_Start_Time . ' UTC and ' . $Conflicted_End_Time . ' UTC. Please modify your reservation request and try again.' );
	}
	else
	{
		# get the reservation start time
