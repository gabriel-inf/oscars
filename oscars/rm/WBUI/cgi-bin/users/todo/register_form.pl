...
sub process_user_registration {
    ...
    # validate user input (fairly minimal... Javascript also takes care of
    ...
    if (!$form_params->{user_dn}) {
        ...
        $results{error_msg} =
            'Please use only alphanumeric characters or _ for login name.';
        return( 1, %results );
    }

    if (!$form_params->{password_once} || !$form_params->{password_twice}) {
        $results{error_msg} = 'Please enter the password.';
        return( 1, %results );
    }
    elsif ($form_params->{password_once} ne $form_params->{password_twice} ) {
        $results{error_msg} =
            'Please enter the same password twice for verification.';
        return( 1, %results );
    }
    ...
    open( MAIL, "|$sendmail_binary_path_and_flags $registration_notification_email_toaddr" ) || 
        return( 1, 'Your user registration has been recorded successfully,' .
            ' but sending a notification email to the service administrator' .
            ' has failed. It may take a while longer for the administrator' .
            ' to accept your registration. Please contact the webmaster at ' .
            $webmaster .
    ...
    print MAIL 'Content-Type: text/plain; charset="' .
               $registration_notification_email_encoding .  '"' . "\n\n";
		
    print MAIL $form_params->{firstname} . ' ' . $form_params->{lastname} .
               ' <' . $form_params->{email_primary} . '> has requested a new' .
               ' user account. Please visit the user admin Web page to accept' .
    ...
    close( MAIL ) || return( 1, 'Your user registration has been recorded' .
        ' successfully, but sending a notification email to the service' .
        ' administrator has failed. It may take a while longer for the' .
        ' administrator to accept your registration. Please contact the' .
        ' webmaster at ' . $webmaster . ', and inform the person of the' .
        ' date and time of error.<br>[Error] ' . $! );

    # when everything has been processed successfully...
    return( 0, 'Your user registration has been recorded successfully.' .
        ' Your login name is <strong>' . $form_params->{user_dn} .
