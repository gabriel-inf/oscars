...
$interface_template_filename = 'editprofile.html';
...
sub Process_Profile_Update
{
	...
	if ( $Error_Status != 1 )
	{
		...
		&Print_Interface_Screen( 0, 'An error has occurred while updating the account information.<br>[Error] ' . $Error_Status );
	}
