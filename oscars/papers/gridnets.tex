...
cost-effectively by the shared service production network.  In this context, services must 
...
developed in various coordinated community efforts (e.g. the ESnet, OSCARS;
Internet2, BRUW; GEANT-GN2-JRA3, SA3-PACE, and Advance Multi-domain Provisioning System (AMPS); and DOE/BNL, TeraPaths projects) to ensure compatible network-network interfaces, which are not yet well defined in the standards arena.

OSCARS has been developed and deployed within ESnet,
...
passes this notification back to the user.
...
contains additional routing information such as the ingress and/or egress border routers
within the OSCARS administrative domain.

In the scenario where an ingress border router is not explicitly communicated, 
...
is outside of its administrative domain, OSCARS marks that last router (within its administrative
 control) as the ingress router.

In the scenario where the egress border router is not contained in the reservation requests, 
the destination IP address is used as the target of the traceroute (sourced from the ingress border router).
Using the same method outlined in the previous paragraph, the egress border router can be determined.

With the ingress and egress border routers identified, the path (or route) between the two can be trivially determined.

The need for OSCARS to support reservations with explicit ingress and egress routers is to
facilitate traffic engineering for sites or networks that have more then one peering connection to ESnet.

In the event that the circuit spans multiple administrative domains, OSCARS uses
routing information harvested from the Border Gateway Protocol (BGP) on the egress router to determine the next
Autonomous System (AS) that the request should be forwarded to.   The AS number is
...
Agreement (SLA) between peer networks.  In almost all cases, the usage condition outlined within an SLA determines the maximum aggregate limit.  This implies that individual bandwidth requests are managed by the RM of the originating AS and not propagated independently to the transit ASes (i.e. the transit AS will see the request as coming from the originating AS and not the individual making the request).
...
of intradomain LSPs stiched together at agreed peering points.
...
setting up a path between two hosts that are several Autonomous Systems apart.
For example, imagine a setting up a
...
AS then it originally sent the "Create" request out to. This resulted  to create 
an efficient and heterogeneous WSDL that describes the various
