...
Internet2, BRUW [BRUW]; GEANT-GN2-JRA3 [GN2-JRA3], SA3-PACE, and Advance Multi-domain Provisioning System (AMPS) [AMPS]; and DOE/BNL, TeraPaths [TeraPaths] projects) to ensure compatible network-network interfaces, which are not yet well defined in the standards arena.
...
for communicating between a client and the RM, and WSDL [WSDL] for the service
description. 
...
\item
At the requested start time, the PSS configures the ESnet provider edge (PE) router
...
routing information harvested from the Border Gateway Protocol (BGP) [BGP] on the egress PE router is used to determine the next
...
Agreement (SLA) between peer networks.  In almost all cases, the usage condition outlined within an SLA determines the maximum aggregate limit.  This implies that individual bandwidth requests are managed by the rervation system of the originating AS and not propagated independently to the transit ASes (i.e. the transit AS will see the request as coming from the originating AS and not the individual making the request).
...
an efficient and heterogeneous WSDL was created to describe the various
...
\subsection{Network-Network Interface(Chin)}
