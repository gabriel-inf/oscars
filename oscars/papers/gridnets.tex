...
<<<<<<< .mine
The closest coordination is with the Internet2 BRUW \cite{bruw} system.  A version 
of BRUW was used as the starting point for OSCARS, and now the two projects 
share a common code base. Interoperability testing is on-going with Internet2 
sites. 

A Web Services compliant [WSI-BP]\emph{ Web Services Definition Language}
 \textbf({WSDL}) specification for the network-network 
interface has been specified following the model of GEANT's
Advance Multi-domain Provisioning System (AMPS)  \cite{geant-amps}. This interface is being 
tested with the DOE/BNL, TeraPaths \cite{bradley} project. 
...
Web services standards are used wherever possible.  SOAP [SOAP] messsages are
used for communications between clients and the RM, and 
the Web Services Definition Language [WSDL] is used for the service description. 
...
To facilitate interdomain virtual circuit setup, a WSDL specification for the 
...
another in a well defined manner. It is then up to the individual reservation 
system to transform the various messages to internal calls to reserve and 
...
\bibitem{XACML} T.  Moses (ed), \emph{eXtensible Access Control Markup Language 2 
(XACML) Version 2.0 3 }
...
\bibitem{terapaths}
Terapaths Project Website,  http://www.atlasgrid.bnl.gov/terapaths/
