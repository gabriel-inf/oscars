...
network services on a production network. Some of these challenges are: allowing 
...
is checked to see if authorization is required, and if so, the user's
authorizations are checked to see if they are sufficient.
...
not directly part of the OSCARS implementation team can use familiar tools to check on (or modify)
who has rights to make reservations or control routing on ``his'' network. 

Within OSCARS, access is controlled for 
...
issue.  The typical user does not need special authorization to access
default functionality, as long as they have been authenticated.  Authorization 
checks are only required for the small subset of users at ESnet 
sites who need to request tasks such as administrative actions, creation
of reservations with larger than 
normal bandwidth, or changes in routing configuration. 
