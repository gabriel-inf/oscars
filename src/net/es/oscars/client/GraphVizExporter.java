...
public class GraphVizExporter {
    ...
    /**
     * subgraph cluster_b Where is your dot program located? It will be called
     * externally.
     */
    ...
    public void addLinkToPath(Link link, ArrayList<String> nodesOnPath,
            HashMap<String, ArrayList<String>> nodePortsOnPath) {
        if (link == null) {
            return;
        }
    ...
    public void addResvEdge(
        ...
        if (nextPe != null) {
            ...
            String nextNodeId = nextDom.getTopologyIdent() + "_"
                    + nextNode.getTopologyIdent();
        ...
        if (addToGroup) {
            ArrayList<String> resvEndpointGroup = resvEndpointGroups
                    .get(nodeId);
    ...
    public void addResvEdge(
        ...
        HashMap<String, HashMap<String, String>> resvEdges = griToResvEdges
                .get(gri);
        if (resvEdges == null) {
    ...
    public void exportTopology(Topology topo, List<Reservation> resvs) {
        ...
        HashMap<String, String> graphAttrs = new HashMap<String, String>();
        ...
        // find out which nodes & links we need to print: only those on
        // reservation paths
        for (Reservation resv : resvs) {
            String gri = resv.getGlobalReservationId();
            String resvDescription = resv.getDescription().trim();
            resvDescription = resvDescription
                    .replaceAll("\\[PRODUCTION\\]", "");
            resvDescription = resvDescription.replaceAll(
                    "\\[PRODUCTION CIRCUIT\\]", "");
            String startId = gri + "-start";
            String endId = gri + "-end";
            ...
            /*
             * INTERDOMAIN: FIXME (wasn't previously functional) path =
             * resv.getPath(PathType.INTERDOMAIN); pathElems =
             * path.getPathElems(); for (int i = 0; i < pathElems.size(); i++) {
             * PathElem pe = pathElems.get(i); if (i < pathElems.size()-1) {
             * nextPe = pathElems.get(i+1); } else { nextPe = null; } Link link
             * = pe.getLink(); this.addResvEdge(pe, nextPe, gri, griToResvEdges,
             * true); this.addLinkToPath(link, nodesOnPath, nodePortsOnPath); }
             * Layer2Data l2data = path.getLayer2Data(); if (l2data != null) {
             * String src = l2data.getSrcEndpoint(); String dst =
             * l2data.getDestEndpoint();
             * 
             * Link srcLink = domDAO.getFullyQualifiedLink(src); Link dstLink =
             * domDAO.getFullyQualifiedLink(dst); this.addLinkToPath(srcLink,
             * nodesOnPath, nodePortsOnPath); this.addLinkToPath(dstLink,
             * nodesOnPath, nodePortsOnPath); }
             */
        }
        ...
        for (Domain dom : topo.getDomains()) {
            ...
            Iterator<Node> nodeIt = (Iterator<Node>) dom.getNodes().iterator();
            while (nodeIt.hasNext()) {
                Node node = (Node) nodeIt.next();
                String nodeId = dom.getTopologyIdent() + "_"
                ...
                if (this.nodeIsPrintable(node) && nodesOnPath.contains(nodeId)) {
                    ...
                    Iterator<Port> portIt = (Iterator<Port>) node.getPorts()
                            .iterator();
                    while (portIt.hasNext()) {
                        ...
                        portId = portId.replaceAll("TenGigabitEthernet", "Te");
                        portId = portId.replaceAll("GigabitEthernet", "Ge");
                        ArrayList<String> thisNodePortsOnPath = nodePortsOnPath
                                .get(nodeId);
                        ...
                        if (this.portIsPrintable(port)
                                && thisNodePortsOnPath.contains(portId)) {

                            Iterator<Link> linkIt = (Iterator<Link>) port
                                    .getLinks().iterator();
                            while (linkIt.hasNext()) {
                                ...
                                if (this.linkIsPrintable(link)) {
                                    ...
                                    String remPortId = remPort
                                            .getTopologyIdent();
                                    remPortId = remPortId.replaceAll(
                                            "TenGigabitEthernet", "Te");
                                    remPortId = remPortId.replaceAll(
                                            "GigabitEthernet", "Ge");

                                    String remNodeId = remDom
                                            .getTopologyIdent()
                                            + "_"
                                            + remNode.getTopologyIdent();
                                    ArrayList<String> thatNodePortsOnPath = nodePortsOnPath
                                            .get(remNodeId);
                                    if (nodesOnPath.contains(remNodeId)
                                            && thatNodePortsOnPath
                                                    .contains(remPortId)) {

                                        String left = "\"" + nodeId + "\":\""
                                                + portId + "\"";
                                        String right = "\"" + remNodeId
                                                + "\":\"" + remPortId + "\"";
                                        String edge = left + " -- " + right;
                                        String tem = link
                                                .getTrafficEngineeringMetric();

                                        Long bandwidth = link
                                                .getMaximumReservableCapacity();
                                        Long bwMbps = bandwidth / 1000000;

                                        HashMap<String, String> edgeAttrs = new HashMap<String, String>();
                                        edgeAttrs
                                                .put("Mbps", bwMbps.toString());
        ...
        Iterator<String> gpIt = (Iterator<String>) graphAttrs.keySet()
                .iterator();
        while (gpIt.hasNext()) {
        ...
        String nodesDot = this.getTopoNodesDot(graphNodes, nodesToPorts,
                portPositions);
        this.addln(nodesDot);
        ...
        Iterator<String> egIt = (Iterator<String>) resvEndpointGroups.keySet()
                .iterator();
        while (egIt.hasNext()) {
            ...
            for (String endpoint : endpointGroup) {
                this.addln("<tr><td port=\"" + endpoint + "\">" + endpoint
                        + "</td></tr>");
            }
            this.addln("</table>>];");
        }
        ...
        Iterator<String> griIt = (Iterator<String>) griToResvEdges.keySet()
                .iterator();
        int colorIndex = 0;
        while (griIt.hasNext()) {
            ...
            HashMap<String, HashMap<String, String>> resvEdges = griToResvEdges
                    .get(gri);
            Iterator<String> edgeIt = (Iterator<String>) resvEdges.keySet()
                    .iterator();
    ...
    public String getTopoNodesDot(
            HashMap<String, HashMap<String, String>> graphNodes,
            HashMap<String, ArrayList<String>> nodesToPorts,
            HashMap<String, String> portPositions) {
        // print NODES
        String nodesDot = "{\n";
        nodesDot += "node [shape=none];\n";
        Iterator<String> graphNodeIt = (Iterator<String>) graphNodes.keySet()
                .iterator();
        while (graphNodeIt.hasNext()) {
            String nodeId = graphNodeIt.next();
            String nodeDot = "\"" + nodeId + "\"";
            nodeDot += " [label=<<table cellspacing=\"24\" cellpadding=\"4\">\n";
            nodeDot += " <tr><td colspan=\"2\">" + nodeId + "</td></tr>\n";
            Iterator<String> npIt = (Iterator<String>) nodesToPorts.get(nodeId)
                    .iterator();
            int i = 0;
            while (npIt.hasNext()) {
    ...
    public String getTopoEdgesDot(
            HashMap<String, HashMap<String, String>> graphEdges,
            HashMap<String, String> portPositions) {
        HashMap<String, ArrayList<String>> alreadyLinked = new HashMap<String, ArrayList<String>>();

        String edgesDot = "";
        // print EDGES
        Iterator<String> gedgeIt = (Iterator<String>) graphEdges.keySet()
                .iterator();
        while (gedgeIt.hasNext()) {
            String edgeDot = "";
            String edge = (String) gedgeIt.next();
            HashMap<String, String> edgeAttrs = graphEdges.get(edge);
            String left = edgeAttrs.get("left");
            String right = edgeAttrs.get("right");
            String leftPort = edgeAttrs.get("leftPort");
            String rightPort = edgeAttrs.get("rightPort");
            String leftNode = edgeAttrs.get("leftNode");
            String rightNode = edgeAttrs.get("rightNode");
            String leftDomain = edgeAttrs.get("leftDomain");
            String rightDomain = edgeAttrs.get("rightDomain");
            String leftPortPos = portPositions.get(leftNode + "_" + leftPort);
            String rightPortPos = portPositions
                    .get(rightNode + "_" + rightPort);
            ...
            edgeDot = left + leftPortPos + " -- " + right + rightPortPos
                    + " [dir=both, " + style + ", weight=" + weight + ", len="
                    + length + "];\n";
            edgesDot += edgeDot;
        }
        return edgesDot;
    ...
    public String exportReservations(ResDetails[] resList, String[] topNodes)
            throws IOException {
        ...
        for (int i = 0; i < resList.length; i++) {
            ...
            if (topNodes != null) {
                ...
                Hashtable<String, String> parsed = URNParser
                        .parseTopoIdent(topoId);
            ...
            for (int j = 0; j < hops.length; j++) {
                ...
                Hashtable<String, String> parsed = URNParser
                        .parseTopoIdent(topoId);
                if (parsed == null || parsed.get("type") == null
                        || !parsed.get("type").equals("link")) {
                ...
                if (portBWs.get(portVertexId) == null) {
                    ...
                    portBWs.put(portVertexId, portBWs.get(portVertexId)
                            + bandwidth);
                }
        ...
        // cluster
        Iterator<String> locIt = locationClusters.keySet().iterator();
        while (locIt.hasNext()) {
            String location = locIt.next();
            locCluster = locationClusters.get(location);
            ...
            if (!reqStarts.isEmpty()) {
                ...
                for (String portId : reqStarts) {
                    if (!openSubgraph) {
                        // output +=
                        // "  subgraph \"cluster_"+location+"_reqstarts_"+j+"\" {\n";
                        // output += "    node [shape=record];\n";
                        // output += "    style=\"invis\";\n";
                        openSubgraph = true;
                    }
            ...
            if (!reqEnds.isEmpty()) {
                ...
                for (String portId : reqEnds) {
                    if (!openSubgraph) {
                        // output +=
                        // "  subgraph \"cluster_"+location+"_reqends_"+j+"\" {\n";
                        // output += "    node [shape=record];\n";
                        // output += "    style=\"invis\";\n";
                        openSubgraph = true;
                    }
        ...
        for (ArrayList<String> theHops : coloredHops) {
            ...
            for (int j = 0; j < theHops.size() - 1; j++) {
                ...
                if (j == 0) {
                    output += "\"" + gri + "-start\" -> " + hop + " [color="
                            + color + ",dir=both, style=" + lineweight
                            + ", weight=0.02];\n";
                }
                if (j % 2 > 0) {
                    output += hop + " -> " + nextHop + " [color=" + color
                            + ",dir=both, style=" + lineweight
                            + ", weight=0.02];\n";
                } else {
                    output += hop + " -> " + nextHop + " [color=" + color
                            + ",dir=both];\n";
    ...
    public boolean nodeIsPrintable(Node node) {
        ...
        Iterator<Port> portIt = (Iterator<Port>) node.getPorts().iterator();
        while (portIt.hasNext()) {
    ...
    public boolean portIsPrintable(Port port) {
        ...
        Iterator<Link> linkIt = (Iterator<Link>) port.getLinks().iterator();
        while (linkIt.hasNext()) {
     ...
     * @return A byte array containin g the image of the graph.
     */
    ...
    private byte[] get_img_stream(File dot) {
        ...
        try {
            ...
            String cmd = DOT + " -Tpng " + dot.getAbsolutePath() + " -o"
                    + img.getAbsolutePath();
    ...
    public void transformToXdot(String dot) {
        try {
            File tempDot = File.createTempFile("graph_", ".dot", new File(
                    TEMP_DIR));
            ...
            if (tempDot.delete() == false)
                System.err.println("Warning: " + tempDot.getAbsolutePath()
                        + " could not be deleted!");
    ...
    public File writeDotSourceToFile(String dot, String fileName)
        ...
        try {
            ...
            System.err
                    .println("Error: I/O error while writing the dot source to file!");
            return null;
        }
        return temp;
    }
    ...
    private File writeDotSourceToFile(String str) throws java.io.IOException {
        ...
        try {
            temp = File
                    .createTempFile("graph_", ".dot.tmp", new File(TEMP_DIR));
            FileWriter fout = new FileWriter(temp);
            fout.write(str);
            fout.close();
        } catch (Exception e) {
