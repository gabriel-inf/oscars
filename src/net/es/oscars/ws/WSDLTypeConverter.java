...
public class WSDLTypeConverter {
    ...
    public static OSCARSEvent getOSCARSEvent(EventContent wsEvent, 
            String producerUrl, String subscriptionId) throws BSSException{
        OSCARSEvent event = new OSCARSEvent();
        
        event.setErrorCode(wsEvent.getErrorCode());
        event.setErrorMessage(wsEvent.getErrorMessage());
        event.setSource(wsEvent.getErrorSource());
        event.setProducerUrl(producerUrl);
        event.setSubscriptionId(subscriptionId);
        event.setTimestamp(wsEvent.getTimestamp());
        event.setType(wsEvent.getType());
        event.setUserLogin(wsEvent.getUserLogin());
        
        ResDetails resDetails = wsEvent.getResDetails();
        if(resDetails != null){
            Reservation resv = new Reservation();
            resv.setBandwidth(resDetails.getBandwidth() + 0L);
            resv.setDescription(resDetails.getDescription());
            resv.setEndTime(resDetails.getEndTime());
            resv.setGlobalReservationId(resDetails.getGlobalReservationId());
            resv.setLogin(resDetails.getLogin());
            resv.setStartTime(resDetails.getStartTime());
            resv.setStatus(resDetails.getStatus());
            Path path = WSDLTypeConverter.convertPath(wsEvent.getResDetails().getPathInfo());
    ...
    /**
     * Converts a Hibernate Reservation bean to a resourceReservation type.
     * Useful for preparing messages sent to a Policy service.
     * 
     * @param resv the Reservation bean to convert
     * @param localDomain the domain ID of the local domain
     * @return the converted ReservationResourceType
     * @throws BSSException
     */
    public static ReservationResourceType reservationToResource(Reservation resv, String localDomain) throws BSSException{
        ReservationResourceType resvResource = new ReservationResourceType();
        
        resvResource.setGlobalReservationId(resv.getGlobalReservationId());
        Long mbps = resv.getBandwidth()/1000000;
        int bandwidth = mbps.intValue();
        resvResource.setBandwidth(bandwidth);
        resvResource.setDescription(resv.getDescription());
        resvResource.setStartTime(resv.getStartTime());
        resvResource.setEndTime(resv.getEndTime());
        
        if(resv.getPath(PathType.INTERDOMAIN) != null){
            resvResource.setPathInfo(getPathInfo(resv, PathType.INTERDOMAIN, true, localDomain));
            PathInfo localPathInfo = getPathInfo(resv, PathType.LOCAL, true, localDomain);
            LocalDetails localDetails = new LocalDetails();
            try {
                localDetails.addExtraElement(localPathInfo.getPath().getOMElement(
                        org.ogf.schema.network.topology.ctrlplane.Path.MY_QNAME, 
                        OMAbstractFactory.getOMFactory()));
            } catch (Exception e) {
                throw new BSSException(e.getMessage());
            }
            resvResource.setLocalDetails(localDetails);
        }else{
            resvResource.setPathInfo(getPathInfo(resv, PathType.REQUESTED, true, localDomain));
        }
        return resvResource;
    }
}
