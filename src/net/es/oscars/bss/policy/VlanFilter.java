...
public class VlanFilter implements PolicyFilter {
	...
	private void examineReservation(Reservation resv, String newLogin, Node newIngressNode, Node newEgressNode, Layer2Info layer2Info) throws BSSException {
		this.log.debug("Examining VLANs for reservation "+resv.getGlobalReservationId());
		PathElem ingress = resv.getPath().getPathElem();
		PathElem egress = ingress;
		while (egress.getNextElem() != null) {
			egress = egress.getNextElem();
		}
	...
	private void removeVlan(String linkDescr, String sameUserGRI, Layer2Info layer2Info) throws BSSException{
	...
	private void processEdgeVtags(Link ingressLink, Link egressLink, Layer2Info layer2Info) throws BSSException {
        TypeConverter tc = new TypeConverter();
        L2SwitchingCapabilityData ingressL2scData 	= ingressLink.getL2SwitchingCapabilityData();
        L2SwitchingCapabilityData egressL2scData  	= egressLink.getL2SwitchingCapabilityData();
