...
public class PathSetupReservation extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        this.log.info("PathSetupReservation.start");
        ...
        try {
            ...
            rmiClient.init();
            outputMap = rmiClient.createPath(inputMap, userName);
        } catch (Exception ex) {
            Utils.handleFailure(out, "PathSetupReservation not completed: " + ex.getMessage(), 
                    methodName, null);
            this.log.error("Error calling rmiClient for PathSetupReservation", ex);
            return;
        }
        String errorMsg = (String)outputMap.get("error");
        if (errorMsg != null) {
            Utils.handleFailure(out, errorMsg, methodName, null);
            this.log.error("PathSetupReservation failed: " + errorMsg);
            return;
        }

        JSONObject jsonObject = JSONObject.fromObject(outputMap);
        out.println("/* " + jsonObject + " */");
        this.log.info("PathSetupReservation.end");
    }
