...
public class SdnPSSSoapHandler implements PSSPortType {
	...
	private VlanTag extractVlan(CtrlPlanePathContent path, boolean isSrc) {
	    VlanTag tag = null; 
	    if(path.getHop() == null || path.getHop().isEmpty()){
	        return tag;
	    }
	    
	    int index = (isSrc ? 0 : (path.getHop().size() -1));
	    if(path.getHop().get(index).getLink() != null &&
                path.getHop().get(index).getLink().getSwitchingCapabilityDescriptors() != null &&
                path.getHop().get(index).getLink().getSwitchingCapabilityDescriptors().getSwitchingCapabilitySpecificInfo() != null &&
                path.getHop().get(index).getLink().getSwitchingCapabilityDescriptors().getSwitchingCapabilitySpecificInfo().getVlanRangeAvailability() != null){
	        tag = new VlanTag();
                tag.setValue(path.getHop().get(index).getLink().getSwitchingCapabilityDescriptors().getSwitchingCapabilitySpecificInfo().getVlanRangeAvailability());
                try{
                    tag.setTagged(Integer.parseInt(tag.getValue()) > 0);
                }catch(Exception e){
                    //ignore VLAN ranges due to disabled VLAN selection in PCE
                    tag = null;
                }
            }
	    return tag;
