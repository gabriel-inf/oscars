...
public class OFRule extends HashMap<String, String> {
	...
	public void putVlan(String srcVlan, String dstVlan) {
	        if(dstVlan == null || srcVlan == null){
	            return;
	        }
	        
	        boolean srcIsTagged = true;
	        boolean dstIsTagged = true;
	        //determine if tagged
	        try{
	            int srcVlanInt = Integer.parseInt(srcVlan);
	            int dstVlanInt = Integer.parseInt(dstVlan);
	            if(srcVlanInt <= 0){
	                srcIsTagged = false;
	            }
	            if(dstVlanInt <= 0){
	            ...
	            Log.warn("Vlan not a number: " + e.getMessage());
	        }
	        
	        this.putVlan(srcIsTagged, srcVlan, dstIsTagged, dstVlan);
	}
